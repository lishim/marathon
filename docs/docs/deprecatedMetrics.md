...
When the deprecated metrics are enabled, Marathon uses [Kamon.io](http://kamon.io/)
for its metrics. You can query the metrics via the `/metrics` HTTP endpoint or
...
[metrics command-line flags]({{ site.baseurl }}/docs/command-line-flags.html#metrics-flags) section.
