...
class DataDogUDPReporter(metricsConf: MetricsConf, registry: MetricRegistry) extends Actor {
  private val remote: InetSocketAddress = {
    val host = metricsConf.metricsDataDogHost()
    val port = metricsConf.metricsDataDogPort()
    new InetSocketAddress(host, port)
  }
  private val transmissionIntervalMs: Long = metricsConf.metricsDataDogTransmissionIntervalMs()
